...
inline bool load_sparse_array_1d(const py::handle &src, SparseArray<T> &out,
  ...
  if (!dtype_matches<T>(data) || !index_dtype_matches(indices)) {
  ...
  indices_owner = py::reinterpret_borrow<py::object>(src.attr("indices"));
...
inline bool load_sparse_array_2d(const py::handle &src, SparseArray2d<T, MAJ> &out,
  ...
  py::array data = py::reinterpret_borrow<py::array>(src.attr("data"));
  py::array indices = py::reinterpret_borrow<py::array>(src.attr("indices"));
  py::array indptr = py::reinterpret_borrow<py::array>(src.attr("indptr"));
  if (!dtype_matches<T>(data) || !index_dtype_matches(indices) ||
      !index_dtype_matches(indptr)) {
    PyErr_SetString(PyExc_ValueError, ("Expecting a sparse " +
                                       std::string(numpy_type<T>::name) +
                                       " array").c_str());
    return false;
  ...
  data_owner = py::reinterpret_borrow<py::object>(src.attr("data"));
  indices_owner = py::reinterpret_borrow<py::object>(src.attr("indices"));
  indptr_owner = py::reinterpret_borrow<py::object>(src.attr("indptr"));
