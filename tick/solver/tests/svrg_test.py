...
class SVRGTest(object):
    ...
    def test_seed_minus_one_rebuilds_random_seed_after_explicit_seed(self):
        svrg = SVRG(seed=123, verbose=False)
        self.assertEqual(svrg._get_effective_seed(), 123)
        self.assertEqual(svrg._cpp_seed, 123)

        np.random.seed(987)
        expected_random_seed = np.random.RandomState(987).randint(0, 2 ** 31 - 1)

        svrg.seed = -1
        self.assertIsNone(svrg._cpp_seed)
        self.assertEqual(svrg._get_effective_seed(), expected_random_seed)
        self.assertEqual(svrg._get_effective_seed(), expected_random_seed)
    ...
    def test_multi_solve_matches_individual_solves_with_starting_iterates(self):
        ...
        for actual, target, solver in zip(result, expected, solvers):
            np.testing.assert_array_almost_equal(actual, target, decimal=2)
            np.testing.assert_array_almost_equal(solver.solution, target,
                                                 decimal=2)
